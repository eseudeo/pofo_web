...
<%
	...
	String redirectUrl = Constants.JSP_FINDPW_FORM;
	...
	if (isEmpty(id, name, email))
		redirectCause = "빈칸을 작성하세요";
	else {
		view = FindId.findPw(id, name, email);
		if (view.size() > 0) {
			Map row = view.get(0);
			pw = row.get(UserColumn.PASSWORD.toString()).toString();
		}
	}
%>
<html>
		...
		<div style="width: 40%;">
			<div>
				<%
					if (pw.equals("")) {
				%>
				<p class="lead">비밀번호를 찾지 못했습니다.</p>
				<div>
					<input type="button" value="비밀번호 다시 찾기"  class="btn"
						onclick="openPage('jsp_findpw_form.jsp')"> <input
			...
			<div>
				...
				<p class="lead">고객님의 정보와 일치하는 비밀번호입니다.</p>
				<div class="panel pansel-default">
					<div class="panel-body"><%=pw%></div>
				</div>
				<div>
...
%>
<%!private boolean isEmpty(String id, String name, String email) {
